...
public class ConvolutionFilterOperation 
{
    ...
    public BufferedImage apply()
    {
        ...
        int[] originalPixels = originalImage.getRGB(0, 0, originalWidth, originalHeight, 
                                                                null, 0, originalWidth);
        
        int[] enlargedPixels = PixelUtil.getEnlargedPixels(originalPixels, originalWidth, originalHeight,
                                                        enlargedWidth, enlargedHeight, pivot);
        ...
        for(int i = 0; i < originalHeight; ++i)
        {
            ...
            {
                int pixelValue = applyFilter(i, j, kernel, kernelWidth, kernelHeight, 
                                        enlargedPixels, enlargedWidth, pivot, d);
    ...
    private int applyFilter(int row, int column, int[] kernel, 
            int kernelWidth, int kernelHeight, int[] pixels, 
            int imageWidth, Point pivot, int d)
    {
        int redTotal = 0;
        int greenTotal = 0;
        int blueTotal = 0;
